...
def deployment_view(deployment: dict | None, target_configured: bool) -> dict:
    ...
    text from raw deployment fields itself."""
    if not target_configured:
        return {"state": "NOT_CONFIGURED", "label": "DEV target not configured", "running": False, "deployment": None}
    if not deployment:
        return {"state": "NOT_DEPLOYED", "label": "NOT DEPLOYED", "running": False, "deployment": None}
    status = deployment["status"]
    if status in ACTIVE_STATUSES:
        return {"state": status, "label": PHASE_LABEL.get(status, status), "running": True, "deployment": deployment}
    if status == "VERIFIED":
        return {"state": "VERIFIED", "label": "DEV VERIFIED", "running": False, "deployment": deployment}
    if status == "FAILED":
        return {"state": "FAILED", "label": "DEV FAILED", "running": False, "deployment": deployment}
    if status == "ROLLED_BACK":
        return {"state": "ROLLED_BACK", "label": "ROLLED BACK", "running": False, "deployment": deployment}
    return {"state": status, "label": status, "running": False, "deployment": deployment}
